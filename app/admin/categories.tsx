...
export default function AdminCategories() {
  ...
  if (loading) return <View style={S.center}><ActivityIndicator size="large" color="#ff4757" /></View>;
  ...
  return (
    <View style={S.container}>
      ...
      <Text style={S.count}>{filtered.length} catégorie{filtered.length > 1 ? 's' : ''}</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={<View style={S.empty}><Text style={S.emptyText}>Aucune catégorie</Text></View>}
          ...
          <View>
            ...
            <View style={S.card}>
              ...
              <View style={S.cardBody}>
                ...
                <Text style={S.cardMeta}>/{root.slug}  ·  {getChildren(root.id).length} sous-cat.</Text>
              </View>
              <TouchableOpacity style={S.iconBtn} onPress={() => openEdit(root)}>
                <Ionicons name="pencil-outline" size={17} color="#888" />
              </TouchableOpacity>
              <TouchableOpacity style={[S.iconBtn, S.iconBtnDanger]} onPress={() => handleDelete(root.id, root.name)}>
            ...
            {getChildren(root.id).map(child => (
              <View key={child.id} style={S.childRow}>
                ...
                <View style={[S.card, S.childCard]}>
                  ...
                  <TouchableOpacity style={[S.iconBtn, S.iconBtnDanger]} onPress={() => handleDelete(child.id, child.name)}>
                    <Ionicons name="trash-outline" size={17} color="#ff4757" />
                  </TouchableOpacity>
                </View>
              </View>
          ...
          <View style={S.sheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
                ...
                >
                  <Text style={[S.chipText, !form.parent_id && S.chipTextActive]}>Aucune (racine)</Text>
                </TouchableOpacity>
                {categories
                  ...
                  .map(c => (
                    ...
                    >
                      <Text style={[S.chipText, form.parent_id === c.id && S.chipTextActive]}>{c.name}</Text>
                    </TouchableOpacity>
                  ))}
              </ScrollView>
...
function FieldLabel({ text }: { text: string }) {
  return <Text style={{ fontSize: 12, color: '#555', fontWeight: '600', marginBottom: 6 }}>{text}</Text>;
}

const C = { bg: '#FBFCFD', surface: '#FFFFFF', border: '#F2F2F7', text: '#1C1C1E', muted: '#8E8E93', accent: '#007AFF' };
const S = StyleSheet.create({
  container:     { flex: 1, backgroundColor: C.bg },
  center:        { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: C.bg },
  topBar:        { flexDirection: 'row', padding: 12, gap: 10, borderBottomWidth: 1, borderColor: C.border },
  searchWrap:    { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: C.surface, borderRadius: 10, paddingHorizontal: 12, borderWidth: 1, borderColor: C.border },
  searchInput:   { flex: 1, height: 42, color: C.text, fontSize: 14 },
  btnAdd:        { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: C.accent, borderRadius: 10, paddingHorizontal: 14, height: 42 },
  ...
  card:          { flex: 1, flexDirection: 'row', alignItems: 'center', backgroundColor: C.surface, borderRadius: 14, padding: 14, borderWidth: 1, borderColor: C.border, gap: 10 },
  ...
  parentIcon:    { width: 34, height: 34, borderRadius: 10, backgroundColor: '#ff47571a', justifyContent: 'center', alignItems: 'center' },
  childIcon:     { width: 28, height: 28, borderRadius: 8, backgroundColor: '#1a1a1a', justifyContent: 'center', alignItems: 'center' },
  ...
  fab:           { position: 'absolute', bottom: 28, right: 20, width: 60, height: 60, borderRadius: 30, backgroundColor: C.accent, justifyContent: 'center', alignItems: 'center', elevation: 8 },
  ...
  input:         { backgroundColor: C.surface, borderRadius: 10, padding: 13, color: C.text, fontSize: 14, borderWidth: 1, borderColor: C.border, marginBottom: 4 },
  chip:          { borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8, backgroundColor: C.surface, borderWidth: 1, borderColor: C.border, marginRight: 8 },
  chipActive:    { backgroundColor: C.accent, borderColor: C.accent },
  chipText:      { color: C.muted, fontSize: 13, fontWeight: '600' },
  chipTextActive:{ color: '#fff' },
  btnSave:       { backgroundColor: C.accent, borderRadius: 12, padding: 16, alignItems: 'center', marginTop: 28 },
