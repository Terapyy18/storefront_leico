...
import { COLORS, S, STATUS_MAP } from './styles.order';
  ...
  async function addItem(variant: Variant) {
    ...
    if (existing) {
      ...
      const { data, error } = await supabase
        .from('order_item')
        .insert({ order_id: selected.id, variant_id: variant.id, quantity: 1, unit_price: variant.product.price })
        .select(`id, quantity, unit_price, variant_id,
          product_variant ( id, sku, size, color, product ( name ) )`)
        .single();
      if (!error && data) {
    ...
    <View style={S.container}>
      ...
      >
        ...
        {STATUSES.map(s => (
          <TouchableOpacity
            key={s.key}
            style={[S.filterPill, filterStatus === s.key && { backgroundColor: s.bg, borderColor: s.dot }]}
            onPress={() => setFilter(filterStatus === s.key ? null : s.key)}
          >
            <View style={[S.filterPillDot, { backgroundColor: s.dot }]} />
            <Text style={[S.filterPillText, filterStatus === s.key && { color: s.text, fontWeight: '700' }]}>
      ...
      <View style={S.countBar}>
        <Text style={S.countText}>{filtered.length} résultat{filtered.length > 1 ? 's' : ''}</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const st = STATUS_MAP[item.status] ?? { bg: '#F3F4F6', text: '#374151', dot: '#9CA3AF', label: item.status };
          return (
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        ...
        <View style={S.sheet}>
          ...
          <ScrollView style={S.sheetBody} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={S.statusScrollContent}>
              {STATUSES.map(s => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[S.statusSelectorBtn, { borderColor: s.dot, backgroundColor: isActive ? s.dot : 'transparent' }]}
                    onPress={() => updateStatus(s.key)}
                  >
                    {updatingStatus && isActive
                      ? <ActivityIndicator size="small" color="#fff" />
                      : <Text style={[S.statusSelectorText, { color: isActive ? '#fff' : s.dot }]}>{s.label}</Text>
                    }
                  </TouchableOpacity>
                );
              })}
            ...
            {selected?.shipping_address ? (
              <>
                ...
                <View style={S.addressBox}>
                  <Ionicons name="location-outline" size={16} color={COLORS.textLight} style={{ marginTop: 1 }} />
              ...
              orderItems.map(item => (
                <View key={item.id} style={S.itemCard}>
                  <View style={S.itemTopRow}>
                    <Text style={S.itemName}>{item.product_variant?.product?.name ?? 'Produit inconnu'}</Text>
                  ...
                  <View style={S.itemBottomRow}>
                    ...
                    <View style={S.stepper}>
                      <TouchableOpacity style={S.stepperBtn} onPress={() => updateQty(item, -1)} disabled={item.quantity <= 1}>
        ...
        <View style={[S.sheet, { maxHeight: '80%' }]}>
          ...
          {loadingVariants ? (
            ...
            <FlatList
              ...
              ListEmptyComponent={<View style={S.empty}><Text style={S.emptyText}>Aucun variant trouvé</Text></View>}
              renderItem={({ item: variant }) => {
                const alreadyIn = orderItems.find(i => i.variant_id === variant.id);
                const isAdding  = addingVariantId === variant.id;
                return (
                  <TouchableOpacity style={S.variantRow} onPress={() => addItem(variant)} disabled={isAdding} activeOpacity={0.75}>
                    ...
                    {isAdding
                      ...
                      : alreadyIn
                        ? <View style={S.alreadyBadge}><Text style={S.alreadyText}>+1</Text></View>
                        : <View style={S.addBadge}><Ionicons name="add" size={18} color={COLORS.accent} /></View>
                    }
                  </TouchableOpacity>
                );
              }}
